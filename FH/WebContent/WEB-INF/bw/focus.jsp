	...
	<div class="container main portfolio4column">
        ...
        <div class="sixteen columns row portfolio_filter">
        	<ul>
                ...
                <li><a  href="<%=basePath%>user/lxd/showFocus.action?page=1" style="color:#FF0000;">我关注了谁</a><span>|</span></li>
		...
		<div class="sixteen columns row teasers portfolio">
              ...
              <c:forEach  items="${focusList}"  var="fl" varStatus="vs" >
              <div class="four columns teaser all-group web-group">
          
               <a target='_BLANK' href="<%=basePath%>user/lxd/toUserInfo.action?userId=${fl.userId}&toUserId=${fl.toUserId}" data-text="查看详细资料" class="hovering">
              
               <c:if test="${fl.photoEntity.URL!=null&&fl.photoEntity.URL!=''}">

				
				<img src="<%=basePath%>${fl.photoEntity.URL}" alt="" class="scale-with-grid" />
				</c:if>
				<c:if test="${fl.photoEntity.URL==null||fl.photoEntity.URL==''}">
               ...
               <div class="pluswrap">
                   ...
                   <div class="topline">最近上线时间：${fl.userEntity.lgTime}</div>
                   <div class="subline"><a target='_BLANK' href="<%=basePath%>user/lxd/toUserInfo.action?userId=${fl.userId}&toUserId=${fl.toUserId}">查看资料</a></div>
               </div>
            </div>
              </c:forEach>
        ...
        <div class="clear"></div>
        <div>
       <input type="hidden" id="page" value="${focusCurrentPage}">
		<input type="hidden"  id="totalPage" value="${focusTotalPage}">
        <span>当前页数：</span><span id="page">${focusCurrentPage}</span>
        
        <span>总页数：</span><span id="totalPage">${focusTotalPage}</span>
       
              <a
				href="<%=basePath%>user/lxd/showFocus.action?page=1" onclick="return first()">首页</a>
			<a
				href="<%=basePath%>user/lxd/showFocus.action?page=${focusCurrentPage-1}" onclick="return previous()">上一页</a>
			<a
				href="<%=basePath%>user/lxd/showFocus.action?page=${focusCurrentPage+1}" onclick="return next()">下一页</a>
			<a
				href="<%=basePath%>user/lxd/showFocus.action?page=${focusTotalPage}" onclick="return last()">末页</a>
        </div>
        <!-- News
		================================================== -->
